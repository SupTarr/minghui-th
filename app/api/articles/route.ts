...
export async function GET(req: Request) {
  try {
    ...
    // One-time lazy backfill: for any in-range day that has no per-day index yet
    // (e.g. data created under the old global index.json), derive it from the
    // legacy global index, serve it, and persist a per-day index so the global
    // file is never read for that day again. Global is read at most once here,
    // and only while un-migrated days are still being visited.
